...
class Solution {
public:
    bool solve(vector<int> &v,int d,int capacity)
    {
    ...
    int shipWithinDays(vector<int>& w, int D) {
        int i,j,k,l,lo=INT_MIN,hi=0,n=w.size();
        for(i=0;i<n;i++)
        {
            lo=max(lo,w[i]);
            hi+=w[i];
        ...
        {
            int mid = (lo+hi)/2;
            if(solve(w,D,mid)) hi=mid;// D days se km time lg rha h
            else
                lo=mid+1;// d days se zada lg rha h uss capacity se
        }
        return lo;
    }
};