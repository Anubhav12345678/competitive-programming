...
class Solution {
public:
    int search(vector<int> &v,int st,int en,int target)
    {
        int i;
        for( i=st;i<=en;i++)
            if(v[i]==target)
    ...
    TreeNode *solveutil(vector<int> &in,vector<int> &post,int st,int en,int *pind)
    {
        if(st>en)
            return NULL;
        ...
        res->right = solveutil(in,post,ind+1,en,pind);
        res->left= solveutil(in,post,st,ind-1,pind);
        return res;
    ...
    TreeNode *solve(vector<int> &in,vector<int> &post)
    {
        int n = in.size();
        int pind = n-1;
        return solveutil(in,post,0,n-1,&pind);
    }
